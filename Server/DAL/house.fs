...
type IHouseRepo () = 
  ...
  member _.addHouse (conn: NpgsqlConnection) (house:'T) (user:User)= async {
    ...
    ignore <| command.Parameters.Add(command.CreateParameter(ParameterName="user_id", Value=user.Id))
