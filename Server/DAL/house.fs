...
type IHouseRepo () = 
  ...
  member _.addHouse (conn: NpgsqlConnection) (house:'T) = async {
    use command = conn.CreateCommand()
    command.CommandText <- "
      INSERT INTO houses (
        id, userId, bedroom, bathroom, sqFt, streetNum, city, state, zip
      ) VALUES (
    ...
    ignore <| command.Parameters.Add(command.CreateParameter(ParameterName="user_id", Value=System.Guid()))
    ...
    ignore <| command.Parameters.Add 
        (command.CreateParameter (ParameterName="street_number", Value=house.address.streedNum))
